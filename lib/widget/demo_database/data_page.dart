...
class _DataPageState extends State<DataAppPage> {
...
class _HandleSPDataWidgetState extends State<HandleSPDataWidget> {
  ...
  _update() async {
    ...
    prefs.setString(PWD, '122212');

    setState(() {
      _result = '_update 成功，用户名修改为Paul，密码修改为122212, 请点击查验证结果';
    });
  }
...
class _HandleSQLiteDataWidgetState extends State<HandleSQLiteDataWidget> {
  ...
  _add() async {
    ...
    setState(() {
      _result = '插入username=$username, pwd= $pwd';
    });
  }
  ...
  _delete() async {
    ...
    String sql = 'DELETE FORM user_table WHERE id = ?';
  ...
  _update() async {
    ...
    int count = await db.rawUpdate(sql, ["222222", '1']);
    print(count);
    await db.close();
...
class _HandleFileDataWidgetState extends State<HandleFileDataWidget> {
  ...
  void _requestExtenalStorageDirectory() async {
  ...
  void initState() {
    ...
    _requestExtenalStorageDirectory();
