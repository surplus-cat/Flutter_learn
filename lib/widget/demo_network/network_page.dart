...
import 'package:flutter_learn/model/hot_news_model.dart';
...
class _NetWorkAppPageState extends State<NetWorkAppPage> {
  ...
  _decodeTest(var body) {
    ...
    List topStories = body['topStories'];
    ...
    List<HotNewsTopStoriesModel> topStoriesList = topStories.map((model) {
      return new HotNewsTopStoriesModel.fromJosn(model);
    }).toList();

    HotNewsModel hotNewsModel = new HotNewsModel(
      date: date, stories: stories, top_stories: topStoriesList
    );
